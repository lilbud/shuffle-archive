...
For example, the content of each post (in the JSON file) is stored as HTML. The HTML needs a bit of pre-processing, mainly fixing certain tags or replacing certain unicode bits. (see: `fixes.txt`). Once this is done, the resulting HTML can be converted using Calibres `ebook-convert` command line tool. This results in a clean ebook file for each post.
