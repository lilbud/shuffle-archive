...
As for media, all of those files will be stored in a top level "Assets" folder (following Jekyll's standard). I considered storing media with each post, but if certain assets are used in multiple posts then those files will be duplicated. Keeping everything in a single folder reduces the need for duplication.